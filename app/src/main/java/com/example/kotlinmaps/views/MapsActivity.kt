package com.example.kotlinmaps.views
...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback,GoogleMap.OnMapLongClickListener {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        if(info=="new") {
            ...
            //Konum iznini kabul etmemişse izin vermesi için yapılan yol
            if (ContextCompat.checkSelfPermission(this,Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                if(ActivityCompat.shouldShowRequestPermissionRationale(this,Manifest.permission.ACCESS_FINE_LOCATION)) {
                    Snackbar.make(binding.root,"Permissions need for location",Snackbar.LENGTH_INDEFINITE).setAction("Give Permission") { location ->
    ...
    fun initListener() {
        binding.saveButton.setOnClickListener {
            if(selectedLatitude != null && selectedLongitude !=null) {
                ...
                compositeDisposable.add(placeDao.insert(place).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(
                    this::handleResponse
                ))
            }
        }
